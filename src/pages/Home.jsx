...
const PackageCard = ({ pkg, onViewDetails }) => (
    <motion.div
        ...
        className="bg-white rounded-xl sm:rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 group border border-gray-100 flex flex-col h-full"
    >
        <div className="relative h-28 xs:h-36 sm:h-52 overflow-hidden">
            <img
                src={pkg.img}
                alt={pkg.title}
            ...
            <span className="absolute top-2 left-2 bg-brand-gold text-brand-dark text-[8px] sm:text-[11px] font-black px-1.5 sm:px-2.5 py-0.5 sm:py-1 rounded-full tracking-wider">{pkg.tag}</span>
            <div className="absolute bottom-2 left-2 flex items-center gap-0.5 sm:gap-2 text-white text-[8px] sm:text-xs">
                <Star size={10} className="fill-brand-gold text-brand-gold shrink-0 w-2.5 h-2.5 sm:w-3 sm:h-3" /> {pkg.rating} <span className="hidden xs:inline">({pkg.reviews})</span>
            </div>
        </div>
        <div className="p-2.5 sm:p-5 flex flex-col flex-1">
            <div className="flex items-center gap-1 text-[10px] sm:text-sm text-gray-400 mb-1 sm:mb-2">
                <MapPin size={10} className="text-brand-gold shrink-0 w-2.5 h-2.5 sm:w-[13px] sm:h-[13px]" /> <span className="truncate">{pkg.location}</span>
            </div>
            <h3 className="font-bold text-brand-dark text-xs sm:text-lg mb-1 sm:mb-2 font-serif leading-tight group-hover:text-brand-gold transition-colors line-clamp-2 min-h-[2rem] sm:min-h-[2.5rem]">{pkg.title}</h3>
            <div className="flex items-center gap-3 text-[10px] sm:text-sm text-gray-500 mb-3 sm:mb-4">
                <span className="flex items-center gap-1"><Clock size={10} className="shrink-0 w-2.5 h-2.5 sm:w-[13px] sm:h-[13px]" /> {pkg.days}</span>
            </div>
            <div className="mt-auto flex flex-col min-[480px]:flex-row min-[480px]:items-center justify-between gap-1.5 sm:gap-2 pt-2 sm:pt-3 border-t border-gray-100">
                <div>
                    <span className="text-[8px] sm:text-xs text-gray-400">Starting from</span>
                    <div className="text-sm sm:text-2xl font-black text-brand-gold font-serif leading-none mt-0.5">{formatPrice(pkg.price)}</div>
                </div>
                <button
                    onClick={() => onViewDetails(pkg)}
                    className="bg-brand-dark text-white px-2 sm:px-4 py-1.5 sm:py-2 rounded-lg text-[9px] sm:text-sm font-bold hover:bg-brand-gold hover:text-brand-dark transition-colors flex items-center justify-center gap-1 cursor-pointer w-full min-[480px]:w-auto"
                >
                    <span className="hidden min-[480px]:inline">View Details</span>
                    <span className="min-[480px]:hidden">View</span>
                    <ArrowRight size={12} className="shrink-0 w-3 h-3 sm:w-3.5 sm:h-3.5" />
                </button>
            </div>
        </div>
    </motion.div>
    ...
    <section className="py-12 sm:py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-8 sm:mb-14">
                ...
                <p className="text-gray-500 mt-4 max-w-xl mx-auto text-xs sm:text-base">Battle-tested itineraries for families, couples, pilgrims, and thrill seekers who want smooth execution and standout experiences.</p>
            </div>
            ...
            <div className="text-center w-full">
                <Link to="/tour-packages" className="w-full sm:w-auto inline-flex justify-center items-center gap-2 border-2 border-brand-dark text-brand-dark font-bold px-8 py-3 rounded-lg hover:bg-brand-dark hover:text-white transition-colors text-sm sm:text-base">
        ...
        <section className="py-12 sm:py-20 bg-white">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="text-center mb-8 sm:mb-14">
                    ...
                    <div className="w-16 h-1 bg-brand-gold mx-auto mt-4 rounded-full"></div>
                    <p className="text-gray-500 mt-4 max-w-xl mx-auto text-xs sm:text-base">Secure premium departures before they fill up. Early booking gets better inventory and stronger savings.</p>
                </div>
                    ...
                    {deals.map((pkg, i) => {
                        ...
                        return (
                            <motion.div
                                ...
                                className="relative rounded-xl sm:rounded-2xl overflow-hidden shadow-md hover:shadow-lg transition-all duration-300 group cursor-pointer"
                                onClick={() => onViewDetails(pkg)}
                            >
                                <div className="relative h-32 sm:h-56">
                                    ...
                                    <span className="absolute top-2 right-2 bg-brand-gold text-brand-dark text-[8px] sm:text-xs font-black px-1.5 sm:px-3 py-0.5 sm:py-1 rounded-full">{discount}% OFF</span>
                                </div>
                                <div className="absolute bottom-0 left-0 right-0 p-2.5 sm:p-5 text-white">
                                    <p className="text-white/70 text-[8px] sm:text-xs mb-1 flex items-center gap-1"><Clock size={10} className="w-3 h-3 sm:w-3.5 sm:h-3.5" /> {pkg.days}</p>
                                    <h3 className="text-white font-bold font-serif mb-1 sm:mb-2 text-xs sm:text-lg line-clamp-2 min-h-[1.75rem] sm:min-h-[2.5rem]">{pkg.title}</h3>
                                    <div className="flex items-end gap-1.5 sm:gap-3">
                                        <span className="text-sm sm:text-2xl font-black text-brand-gold leading-none">{formatPrice(pkg.price)}</span>
                                        <span className="text-white/40 line-through text-[8px] sm:text-sm leading-none">{formatPrice(pkg.originalPrice)}</span>
                                    </div>
                                    <div className="mt-2 flex items-center gap-1 text-brand-gold text-[10px] sm:text-sm font-bold opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity">
                                        View Details <ArrowRight size={12} className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                                    </div>
                                </div>
                            </motion.div>
                        );
...
const AdventureActivities = () => (
    <section className="py-12 sm:py-20 bg-brand-dark relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div className="text-center mb-8 sm:mb-14">
                ...
                <div className="w-16 h-1 bg-brand-gold mx-auto mt-4 rounded-full"></div>
                <p className="text-white/60 mt-4 max-w-xl mx-auto text-xs sm:text-base">From white-water rapids to high-altitude trails, every activity is handpicked for safety, intensity, and unforgettable energy.</p>
            </div>
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6 mb-10">
                {ACTIVITIES.map((act, i) => (
                    <Link
                        key={i}
                        to={act.path || '/activity'}
                    ...
                    >
                        <motion.div
                            ...
                            className="bg-white/5 border border-white/10 backdrop-blur-sm rounded-xl sm:rounded-2xl overflow-hidden hover:bg-white/10 transition-all flex flex-col h-full cursor-pointer"
                        >
                            ...
                            <div className="p-2.5 sm:p-5 flex flex-col flex-1">
                                <h3 className="text-sm sm:text-xl font-bold text-white font-serif mb-0.5 sm:mb-1 line-clamp-1">{act.name}</h3>
                                <p className="text-brand-gold text-[8px] sm:text-xs font-bold uppercase tracking-wide mb-1.5 sm:mb-2 flex items-center gap-1"><MapPin size={10} className="w-2.5 h-2.5 sm:w-3 sm:h-3" />{act.location}</p>
                                <p className="text-white/60 text-[10px] sm:text-sm mb-3 sm:mb-4 line-clamp-2 min-h-[1.5rem] sm:min-h-[2.5rem]">{act.desc}</p>
                                <div className="mt-auto flex items-center justify-between">
                                    <span className="text-brand-gold font-black text-xs sm:text-lg leading-none">{act.price}</span>
                                    <span className="text-white/70 text-[10px] sm:text-sm font-bold group-hover:text-brand-gold transition-colors flex items-center gap-0.5 sm:gap-1">
                                        Book <ArrowRight size={12} className="w-3 h-3 sm:w-3.5 sm:h-3.5 transition-transform group-hover:translate-x-1" />
                                    </span>
                                </div>
                            </div>
                        </motion.div>
            ...
            <div className="text-center">
                <Link to="/activity" className="w-full sm:w-auto inline-flex justify-center items-center gap-2 bg-brand-gold text-brand-dark font-bold px-8 py-3 rounded-lg hover:bg-yellow-400 transition-colors shadow text-sm sm:text-base">
    ...
    <section className="py-12 sm:py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-8 sm:mb-14">
                ...
                <div className="w-16 h-1 bg-brand-gold mx-auto mt-4 rounded-full"></div>
                <p className="text-gray-500 mt-4 max-w-xl mx-auto text-xs sm:text-base">Verified vehicles and experienced mountain drivers for smooth, safe movement from city roads to steep hill routes.</p>
            </div>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6 mb-14">
                {vehicles.map((v, i) => (
                    <Link
                        key={i}
                        to={`/services/car-rental?select=${encodeURIComponent(v.name)}`}
                        state={{ selectVehicle: v.name }}
                        className="block group"
                    >
                        <motion.div
                            initial={{ opacity: 0, y: 20 }}
                            ...
                            viewport={{ once: true }}
                            transition={{ delay: i * 0.1 }}
                            className="bg-white border border-gray-100 rounded-xl sm:rounded-2xl overflow-hidden hover:shadow-lg transition-shadow flex flex-col h-full cursor-pointer"
                        >
                            <div className="relative h-24 sm:h-40 overflow-hidden">
                            ...
                            <div className="p-2.5 sm:p-5 text-center flex flex-col flex-1">
                                <div>
                                    <span className="text-[8px] sm:text-xs bg-brand-gold/20 text-brand-gold font-bold px-1.5 sm:px-2.5 py-0.5 sm:py-1 rounded-full">{v.badge}</span>
                                </div>
                                <h3 className="text-sm sm:text-xl font-bold text-brand-dark mt-2 sm:mt-3 font-serif line-clamp-1 group-hover:text-brand-gold transition-colors">{v.name}</h3>
                                <p className="text-gray-500 text-[10px] sm:text-sm mb-2 sm:mb-4">{v.type}</p>
                                <div className="mt-auto flex justify-center gap-1 sm:gap-2 flex-wrap">
                                    {v.tags.map((t, j) => (
                                        <span key={j} className="text-[8px] sm:text-xs bg-gray-100 text-gray-600 font-semibold px-1.5 sm:px-2.5 py-0.5 sm:py-1 rounded-full">{t}</span>
                                    ))}
                                </div>
                            </div>
                        </motion.div>
    ...
    <section className="py-12 sm:py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-8 sm:mb-14">
                ...
                <div className="w-16 h-1 bg-brand-gold mx-auto mt-4 rounded-full"></div>
                <p className="text-gray-500 mt-4 max-w-xl mx-auto text-xs sm:text-base">Choose your vibe and let us engineer the route, stay, and activities around your exact travel goals.</p>
            </div>
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6">
                {themes.map((theme, i) => (
                    <motion.div
                        ...
                        transition={{ delay: i * 0.08 }}
                    >
                        <Link to={theme.link} className="block rounded-xl sm:rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-shadow group relative">
                            <div className="relative h-32 sm:h-52 overflow-hidden">
                                ...
                                <div className="absolute bottom-0 left-0 right-0 p-2.5 sm:p-5">
                                    <h3 className="text-white text-sm sm:text-2xl font-black font-serif mb-0.5 sm:mb-1">{theme.label}</h3>
                                    <p className="text-white/70 text-[9px] sm:text-sm line-clamp-2 min-h-[1.5rem] sm:min-h-[2.5rem]">{theme.desc}</p>
                                    <div className="flex items-center gap-1 mt-1.5 sm:mt-3 text-brand-gold text-[9px] sm:text-sm font-bold tracking-wider sm:tracking-normal uppercase sm:capitalize">
                                        Explore <ArrowRight size={12} className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                                    </div>
                                </div>
                            </div>
                        </Link>
