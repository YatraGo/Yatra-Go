        ...
        <div className="space-y-10">
            <div className="admin-glass p-8 lg:p-10">
                <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-8 mb-12">
                    <div>
                        <div className="inline-flex items-center gap-2 rounded-full bg-brand-blue/10 px-4 py-2 text-[10px] font-black uppercase tracking-[0.3em] text-brand-blue">
                             <Clock size={14} /> Reservation Desk
                        </div>
                        <h2 className="mt-6 text-3xl font-serif font-black text-brand-dark">Manage Bookings</h2>
                        <p className="text-xs font-bold text-slate-700 mt-2 uppercase tracking-widest leading-relaxed">{bookings.length} reservations currently in pipeline</p>
                    </div>

                    <div className="flex flex-col sm:flex-row gap-4 w-full xl:w-auto">
                        <div className="relative flex-1 sm:w-80 group">
                            <Search className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-600 group-focus-within:text-brand-blue transition-colors" size={20} />
                            <input 
                                type="text" 
                                value={searchTerm} 
                                onChange={(e) => setSearchTerm(e.target.value)}
                                placeholder="Search reservations..." 
                                className="w-full rounded-2xl border border-slate-200 bg-white/50 pl-14 pr-6 py-4 text-xs font-black uppercase tracking-widest outline-none transition-all focus:bg-white focus:border-brand-blue shadow-sm" 
                            />
                        </div>
                        <div className="relative group">
                            ...
                            <select 
                                ...
                                className="w-full sm:w-48 rounded-2xl border border-slate-200 bg-white/50 pl-14 pr-10 py-4 text-[10px] font-black uppercase tracking-widest outline-none transition-all focus:bg-white focus:border-brand-blue shadow-sm appearance-none cursor-pointer"
                            >
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                                            <div className="space-y-1.5">
                                                <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-700">
                                                    <UserRound size={12} className="text-brand-blue" /> Traveler
                                                </div>
                                                <p className="font-serif font-black text-slate-900 truncate">{booking.customerName || 'Explorer'}</p>
                                            </div>
                                            <div className="space-y-1.5">
                                                <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-700">
                                                    <Mail size={12} className="text-brand-gold" /> Correspondence
                                                </div>
                                                <p className="text-xs font-bold text-slate-700 truncate">{booking.customerEmail || 'No Email'}</p>
                                            </div>
                                            <div className="space-y-1.5">
                                                <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-700">
                                                    <MapPin size={12} className="text-emerald-500" /> Target
                                                </div>
                                                <p className="font-serif font-black text-slate-900 truncate">{booking.destination || 'Uttarakhand'}</p>
                                            </div>
                                            <div className="space-y-1.5">
                                                <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-700">
                                                    <CalendarDays size={12} className="text-orange-500" /> Dates
                                                </div>
                                                <p className="font-serif font-black text-slate-900 whitespace-nowrap">{booking.travelDate || 'Discussion'}</p>
                                            </div>
                                        </div>

                                        {booking.notes && (
                                            <div className="rounded-2xl bg-brand-gold/5 border border-brand-gold/10 p-5 text-sm font-medium text-slate-600 leading-relaxed italic relative">
                                                <Sparkles size={14} className="absolute -top-2 -left-2 text-brand-gold" />
                                                {booking.notes}
                                            </div>
                                        )}
                                        
                                        <div className="text-[9px] font-black uppercase tracking-[0.4em] text-slate-700">REQ_ID: #{booking.id.slice(0,10)}</div>
                                    </div>

                                    <div className="flex flex-col gap-6 shrink-0 w-full xl:w-64 border-l border-slate-50 xl:pl-8">
                                        <div className="rounded-[1.5rem] bg-slate-100/50 p-5 border border-slate-200 group-hover:bg-brand-blue/5 transition-colors">
                                            <div className="flex items-center justify-between mb-4">
                                                <span className="text-[10px] font-black uppercase tracking-widest text-slate-700">Group Size</span>
                                                <UserRound size={14} className="text-brand-blue" />
                                            </div>
                                            <div className="font-serif font-black text-2xl text-slate-900">{booking.travelers || 1} <span className="text-xs text-slate-700 uppercase tracking-widest">Persons</span></div>
                                        </div>
                                        
                                        <div className="space-y-3">
                                            <label className="text-[9px] font-black uppercase tracking-widest text-slate-700 ml-2">Pipeline Status</label>
                                            <div className="relative group/sel">
                                                <select 
                                                    value={booking.status || 'pending'} 
                                                    onChange={(e) => updateStatus(booking.id, e.target.value)} 
                                                    className="w-full rounded-2xl border border-slate-200 bg-white px-5 py-4 text-[10px] font-black uppercase tracking-widest outline-none transition-all focus:border-brand-blue shadow-sm appearance-none cursor-pointer"
                                                >
                                                    {statuses.map((status) => (
                                        ...
                                        <button
                                            ...
                                            className="w-full px-4 py-3.5 rounded-xl bg-rose-50 hover:bg-rose-600 text-rose-600 hover:text-white font-bold transition-all duration-300 flex items-center justify-center gap-2 border border-rose-200 hover:border-rose-600 cursor-pointer"
                                        >
                                            <Trash2 size={16} />
                                            Delete Booking
                                        </button>
